<?php
  if (!in_array($userId, $users)){
    ...
    $sql = "SELECT * FROM CONTRACT_GEN_AR WHERE CREATOR_ID = '{$userId}' ORDER BY STATUS, CUST_NAME";
    ...
    $table = "<div class='col-md-2'></div>
      ...
      while($row = $db->fetch($sql))
      {
        ...
        if ($row['STATUS'] == 'APPROVED')   {
          ...
          $table .= "<td style='width: 13.5%;''><a target='_blank' href='contracts/ar/view.php?id=".$row['ID']."&preview=false' id='view_contract_btn' class='btn btn-default btn-sm'>View</a></td></tr>";
        }

        else if ($row['STATUS'] == 'PENDING')  {
          $table .= "<td style='background-color: #fff3e6;'>".$row['STATUS']."</td>";
          $table .= "<td style='width: 13.5%;''><a target='_blank' href='contracts/ar/view.php?id=".$row['ID']."&preview=true' id='view_contract_btn' class='btn btn-default btn-sm'>View</a></td></tr>";
        }
