<?php
  $sql = "SELECT * FROM CONTRACT_GEN_CIO2 WHERE STATUS = 'APPROVED' ORDER BY DATE_SUBMITTED DESC";

  $table = "
      <div class='panel panel-green panel-shadow' style='width: 80%; margin: auto;'>
          <div class='panel-heading'>
            <div class='panel-title'>Approved Contracts</div>
          </div>
    ...
    while($row = $db->fetch($sql))
    {
      $table .= "
          <tr>
            ...
            <td>".$row['PLANNER_REGION']."</td>
            <td>".$row['DATE_APPROVED']."</td>
            <td style='width: 13.5%;''><a target='_blank' href='contracts/ciac/view_contract.php?id=".$row['ID']."&preview=false' id='view_contract_btn' class='btn btn-info btn-sm'>View</a></td>
          </tr>";
    ...
    if($count == 0){
      $table.= "
          <tr>
            <td colspan='7'><div class='alert alert-warning'>There are no approved contracts</div></td>
          </tr>";
    }
